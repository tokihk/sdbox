...
#include "config/xconfig_system.h"
...
static const xuart_config_t XPROMPT_UART_CONFIG =
{
	.baudrate  = 115200,
	.data_bits = 8,
...
static void xprompt_uart_write(const uint8_t *data, size_t data_size, bool_t sync_mode)
{
	size_t write_size;

	do
	{
		write_size = xuart_write(XPROMPT_UART_ID, (const uint8_t *)data, data_size);
...
static void xprompt_vprintf(enum xprompt_attr attr, const char *format, va_list args)
{
	if (   (format != NULL)
		...
		xprompt_uart_write((const uint8_t *)text, text_len, TRUE);
		xprompt_uart_write((const uint8_t *)XPROMPT_NEW_LINE, XCOUNTOF(XPROMPT_NEW_LINE) - 1, TRUE);
	}
}
...
static int ntshell_serial_read(char *buf, int cnt, void *extobj)
{
	...
	/* UARTからデータ取得 */
	read_size = xuart_read(XPROMPT_UART_ID, (uint8_t *)buf, (uint16_t)cnt);
...
{
	return ((int)xuart_write(XPROMPT_UART_ID, (const uint8_t *)buf, (size_t)cnt));
}

void xprompt_init(void)
{
	xuart_open(XPROMPT_UART_ID, &XPROMPT_UART_CONFIG);

	/* ntshell初期化 */
	ntshell_init(&g_xprompt.ntshell_obj, ntshell_serial_read, ntshell_serial_write, ntshell_user_callback, NULL);
	...
	(
			...
			SYSTEM_VERSION_BUGFIX,
			SYSTEM_VERSION_CUSTOM
	);
}

void xprompt_shutdown(void)
{
